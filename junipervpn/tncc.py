...
class TNCC:
    def __init__(self, vpn_host, device_id=None, funk=None, platform=None,
                 ...
                 user_agent=DEFAULT_USER_AGENT, verbose=False, periodic_host_check=False):
        ...
        self.periodic_host_check = periodic_host_check
    ...
    def get_cookie(self, dspreauth=None, dssignin=None):
        ...
        if self.periodic_host_check:
            ...
            try:
                ...
                self.hc_interval = 10
                logging.warning('Unable to extract host check interval! Falling back to default {}'.format(self.hc_interval))

        # We have a new DSPREAUTH cookie
        return self.find_cookie('DSPREAUTH')

class TNCCServer:
    def __init__(self, s, t, periodic_host_check=False):
        self.sock = s
        self.tncc = t
        self.periodic_host_check = periodic_host_check
    ...
    def do_hc(self, sc):
        ...
        delay = (60 * (self.tncc.hc_interval - 1))
        sc.enter(delay, 1, self.do_hc, (sc,))

        logging.info('==== Next Host Checker scheduled in {} seconds'.format(delay))
    ...
    def process_cmd(self):
        ...
        if cmd == 'start':
            ...
            if self.periodic_host_check:
                ...
                delay = (60 * (self.tncc.hc_interval - 1))
                logging.info('==== Received DSPREAUTH Cookie, going into a timed Host Checker loop for {} seconds'.format(delay))

                self.hc_scheduler.enter(delay, 1, self.do_hc, (self.hc_scheduler,))
                self.hc_scheduler.run()
                dsid_value = args['Cookie']
            else:
                logging.warning("Received 'setcookie' (periodic host check command) but --periodic-host-check is not enabled!")
